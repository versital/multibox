...
export function createDebugOverlay() {
    ...
    const attemptCreate = () => {
        console.log("[DEBUG] Attempting to create diagnostic overlay...");
        ...
        overlay.style.cssText = `
            ...
            font-size: 11px; padding: 10px; border: 2px solid #a855f7; 
            border-radius: 8px; z-index: 10000; pointer-events: auto;
            box-shadow: 0 0 15px rgba(168,85,247,0.5); overflow: hidden;
        `;
        document.body.appendChild(overlay);
        console.log("[DEBUG] Diagnostic overlay successfully appended to body.");

        const updateUI = () => {
            overlay.innerHTML = `
                <div style="font-weight: bold; border-bottom: 1px solid #555; margin-bottom: 5px; padding-bottom: 5px; color: #a855f7; display: flex; justify-content: space-between;">
                    <span>🛠️ SYNC DIAGNOSTIC</span>
                    <span style="color: #0f0; cursor: pointer;" onclick="window.dispatchEvent(new CustomEvent('multibox-force-sync'))">FORCE SYNC</span>
                </div>
    ...
    if (document.readyState === "complete" || document.readyState === "interactive") {
        ...
        window.addEventListener("DOMContentLoaded", attemptCreate);
        window.addEventListener("load", attemptCreate);
    }
}
